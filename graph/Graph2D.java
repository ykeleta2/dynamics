 ...
 */
    ...
    public void paint(Graphics g) {
         ...
         Rectangle r = bounds();
         if(!paintAll) return;  // 8/2/97
         if(r.width<=0) return;
         if(r.height<=0) return;
//MCC Next two lines are new to version 2.4
...
//       Make new graphics if bounds have changed or if clearAll true         
         if((! r.equals(r_save) )  ) {                 // clearAll moved 8/2/97
            newRectangle = true;
...
//       Clear and reset axes etc if axes changed or new graphics
         if(axesChanged() || newRectangle || clearAll) {      // clearAll moved 8/2/97
            newRectangle = false;
...
//          Make new copy of offScreenGraphics for clipping etc.                        
        ...
        paintBeforeData(lg,datarect);         // changed r -> datarect (MCC 6/2/97)
        if( !dataset.isEmpty() ) {                
            for (i=0; i<dataset.size(); i++) {
